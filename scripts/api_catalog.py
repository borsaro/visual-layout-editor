...
ENDPOINTS = [
    ...
    {'method': 'POST', 'path': '/api/variants/promote', 'desc': 'Extract one variant as an independent layout and remove it from the set {path, id, filename?}'},
    {'method': 'POST', 'path': '/api/variants/make-base', 'desc': 'Make a variant the base; the old base stays as a variant and the others are rewritten against it {path, id, keep_old_base?}'},
    ...
    {'method': 'GET', 'path': '/api/all-folders', 'desc': 'Every campaign folder, recursive, for move destinations'},
    {'method': 'POST', 'path': '/api/create-folder', 'desc': 'Create a folder {path} relative to the campaigns root'},
    {'method': 'POST', 'path': '/api/move-items', 'desc': 'Move files with their sidecars and tags {items:[paths], dest}'},
    {'method': 'POST', 'path': '/api/set-tags', 'desc': 'Assign tags {items:[{path, tags:[]}]} — stored in per-folder .roby-tags.json'},
